...
git() { printf 'https://github.com/exchgr/exch.gr-11ty.git\n'; }
assert_eq "$(derive_gh_repo)" "exchgr/exch.gr-11ty" "derive_gh_repo strips https github URL"
unset -f git

git() { printf 'git@github.com:exchgr/exch.gr-11ty.git\n'; }
assert_eq "$(derive_gh_repo)" "exchgr/exch.gr-11ty" "derive_gh_repo strips ssh github URL"
unset -f git
...
rc=0
git() { printf 'git@github.com:exchgr/exch.gr-11ty;x.git\n'; }
out="$(derive_gh_repo)" || rc=$?
...
# --- phase_preflight fail-closed on GH_REPO: source-time derivation stays
# tolerant (sourcing the lib must remain spec-safe), so the fail-closed
# enforcement lives inside the phase itself. ---
empty_repo_rc=0
(
  PATH="$pf2/stub:$PATH"
  ...
  GH_REPO=""
  phase_preflight
) > /dev/null 2> "$pf2/empty-repo-err" || empty_repo_rc=$?
assert_status "$empty_repo_rc" 1 "phase_preflight dies when GH_REPO is empty"
assert_contains "$(cat "$pf2/empty-repo-err")" "GH_REPO" \
  "phase_preflight names GH_REPO when the origin could not be derived"
...
(
  ...
  GH_REPO="a//b"
  phase_preflight
) > /dev/null 2> "$pf2/invalid-repo-err" || invalid_repo_rc=$?
assert_status "$invalid_repo_rc" 1 "phase_preflight dies when GH_REPO is not a valid slug"
assert_contains "$(cat "$pf2/invalid-repo-err")" "a//b" \
  "phase_preflight names the offending slug"
