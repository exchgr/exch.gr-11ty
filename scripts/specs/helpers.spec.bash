...
# --- is_vtag ---
rc=0; is_vtag "v4.18.0" || rc=$?
assert_status "$rc" 0 "is_vtag v4.18.0 -> 0"
rc=0; is_vtag "v4" || rc=$?
assert_status "$rc" 0 "is_vtag v4 -> 0"
rc=0; is_vtag "4.18.0" || rc=$?
assert_status "$rc" 1 "is_vtag 4.18.0 -> 1 (missing v prefix)"
rc=0; is_vtag "somebranch" || rc=$?
assert_status "$rc" 1 "is_vtag somebranch -> 1"
rc=0; is_vtag "" || rc=$?
assert_status "$rc" 1 "is_vtag empty -> 1"

# --- dependabot_pkgs ---
fixture='[{"state":"open","dependency":{"package":{"name":"@11ty/eleventy"}}},{"state":"open","dependency":{"package":{"name":"@11ty/eleventy"}}},{"state":"open","dependency":{"package":{"name":"qs"}}},{"state":"dismissed","dependency":{"package":{"name":"nanoid"}}}]'
actual="$(dependabot_pkgs <<<"$fixture")"
assert_eq "$actual" $'@11ty/eleventy\nqs' "dependabot_pkgs distinct open scoped+plain"
