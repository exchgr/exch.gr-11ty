...
# A nonempty but malformed LTS shape must die too: the version is consumed as a
# grep/sed pattern in the pin path, where unvalidated text can silently corrupt
# .tool-versions instead of failing loudly.
il_rc=0
(
  latest_node_lts() { printf 'not-a-version\n'; }
  REPO_DIR="$tmpdir/node-ltsfail" phase_node
) > /dev/null 2> "$tmpdir/node-ltsfail-err3" || il_rc=$?
assert_status "$il_rc" 1 \
  "phase_node aborts when the node LTS lookup returns a non-version string"
assert_contains "$(cat "$tmpdir/node-ltsfail-err3")" "is not a valid node version" \
  "phase_node names the malformed LTS shape on stderr"
assert_eq "$(printf '%s\n' "$(cat "$tmpdir/node-ltsfail/.tool-versions")")" "nodejs 22.0.0" \
  "phase_node leaves .tool-versions untouched when the LTS shape is invalid"
