...
phase_node() {
  ...
  # unlike the soft warn-and-skip lookups in the workflows phase.
  if ! lts_full="$(latest_node_lts)" || [[ -z "$lts_full" ]]; then
    die "node: could not resolve the latest node LTS"
  fi
  # The version feeds grep/sed patterns and asdf arguments below; a malformed
  # value (e.g. a codename or prose) must fail loudly instead of corrupting the
  # pin. Same shape guard workflows.bash applies to the same lookup.
  if ! is_vtag "v$lts_full"; then
    die "node: latest node LTS '$lts_full' is not a valid node version"
  fi
